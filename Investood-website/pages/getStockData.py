...
def getStocks():
    ...
    dataframe = dataframe[['rate','lastprice','hacimstr','code']]
    today = datetime.today().strftime('%Y-%m-%d')
    seq = 1
    for index, row in dataframe.iterrows():
        new_record = Stocks(stock_id=seq,stock_name=row['code'], stock_value=row['lastprice'],stock_volume=row['hacimstr'], stock_rate=row['rate'],stock_date = str(today))
        new_record.save()
        seq +=1
